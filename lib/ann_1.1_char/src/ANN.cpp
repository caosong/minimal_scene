...
ANNpointArray ann_1_1_char::annAllocPts(int n, int dim)		// allocate n pts in dim
{
    ANNpointArray pa = new ANNpoint[n];			// allocate points
    ANNpoint p  = new ANNcoord[(long)n*dim];		// allocate space for coords
    for (int i = 0; i < n; i++) {
        pa[i] = &(p[(long)i*dim]);
    }
    return pa;
...
void ann_1_1_char::annDeallocPts(ANNpointArray &pa)			// deallocate points
{
