...
double align_affine_3D(int num_pts, v3_t *r_pts, v3_t *l_pts, double *Tout) {
    if (num_pts < 4) {
	...
	// free(X);
        ...
        for (i = 0; i < n; i++) {
            ...
            dx = left_prime[0] - Vx(r_pts[i]);
            dy = left_prime[1] - Vy(r_pts[i]);
            dz = left_prime[2] - Vz(r_pts[i]);
