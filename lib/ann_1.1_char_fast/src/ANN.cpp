...
ANNdist ann_1_1_char::annDist(						// interpoint squared distance
	int					dim,
	ANNpoint			p,
	ANNpoint			q)
{
...
void ann_1_1_char::annPrintPt(						// print a point
...
ANNpoint ann_1_1_char::annAllocPt(int dim, ANNcoord c)		// allocate 1 point
{
...
ANNpoint ann_1_1_char::annAllocPts(int n, int dim) // allocate n pts in dim
{
    // ANNpointArray pa = new ANNpoint[n];            // allocate points
    ANNpoint p  = 
        new ANNcoord[(unsigned long long)n*dim];  // allocate space for coords

    if (p == NULL) {
        printf("[annAllocPts] Error allocating space for %llu bytes!\n",
...
void ann_1_1_char::annDeallocPt(ANNpoint &p)					// deallocate 1 point
{
	delete [] p;
	p = NULL;
}
   
void ann_1_1_char::annDeallocPts(ANNpoint &pa) // deallocate points
{
...
ANNpoint ann_1_1_char::annCopyPt(int dim, ANNpoint source)	// copy point
{
	ANNpoint p = new ANNcoord[dim];
...
void ann_1_1_char::annAssignRect(int dim, ANNorthRect &dest, const ANNorthRect &source)
{
	for (int i = 0; i < dim; i++) {
...
void ann_1_1_char::annError(const char *msg, ANNerr level)
{
	if (level == ANNabort) {
...
int	ann_1_1_char::ANNmaxPtsVisited = 0;	// maximum number of pts visited
int	ann_1_1_char::ANNptsVisited;			// number of pts visited in search

//----------------------------------------------------------------------
//	Global function declarations
//----------------------------------------------------------------------

void ann_1_1_char::annMaxPtsVisit(			// set limit on max. pts to visit in search
