...
static ANNkd_ptr annReadDump(			// read dump file
	...
	ANNpoint		&the_pts,				// new points (if applic)
...
#ifdef ANN_1_1_CHAR_DUMP
//  ScottP: ANN Dump/Load Bug
//  Building large ANN trees is a computationally expensive process.  To address this problem,
//  ANN provides APIs to Dump/Load built trees, but some changes were needed to get it
//  working since ANNCoord's type is now an unsigned char (uc) instead of a double.
//
//  Inserting an uc to a stream writes it as a binary byte.  If an uc is inserted with the
//  same value as whitespace (i.e. 10 (\n) or 20 (space)), these will be treated as
//  whitespace delimiters during uc extraction, corrupting data.
//
//  Unfortunately, the dump/load functions inserts and extracts ANNCoord values to/from the
//  stream.  This is not a problem with other data types like double, int, short, etc since
//  they write text to the byte stream.
//
//  Dump/load was fixed by doing two things:
//  1) Overload the uc insertion and extraction operators to use unsigned short
//     This is an ideal change since we don't have to change the dump/load code, but
//     this method uses 2 bytes instead of 1 for storage.
//  2) Improve the speed of the bulk dumping/loading of ANNpoints.  Since each ANNpoint
//     contains 128 ANNCoords, the dump code now writes all 128 ANNCoords at once
//     using ofstream.write(buffer) instead of making 128 uc insertion calls.  Multiply
//     this by 100 million or 1 billion points!  The reverse operation is performed
//     during loading. This provides significant speed improvements and reduces the file
//     size by 2 (compared to using shorts).

// insert unsigned char (binary) as unsigned short (text)
static std::ostream& operator<<(std::ostream &out, unsigned char ob){
  out << static_cast<unsigned short>(ob);
  return out;
}

// extract unsigned char (binary) as unsigned short (text)
static std::istream& operator>>(std::istream &in, unsigned char &ob) {
	unsigned short temp = 0;
	in >> temp;
	ob = static_cast<unsigned char>(temp);
	return in;
}

// Print a Point
static void annPrintPtBatch(unsigned char *pt,				// the point
	int					dim,			// the dimension
	std::ostream		&out)			// output stream
{
	out.write(reinterpret_cast<const char *>(pt), dim);
}

// Read a Point
static void annReadPtBatch(
	std::istream		&in,			// in stream
	unsigned char*		pt,				// the point
	int					dim)			// the dimension
{
	// make sure in has ios::binary set
	in.read(reinterpret_cast<char *>(pt), dim);
}
#endif
		...
		ostream &out)					// output stream
{
    ...
    if (with_pts) {						// print point coordinates
        ...
        for (unsigned long long i = 0; i < (unsigned long long) n_pts; i++) {
            out << i << " ";
#ifdef ANN_1_1_CHAR_DUMP
            // bulk dump case
            annPrintPtBatch(pts + i*dim, dim, out);
#else
		...
		ostream &out)					// output stream
{
    ...
    if (!child[ANN_LO]->isLeaf)
        ((ANNkd_split *)child[ANN_LO])->dump(out);    // print low child
    else 
        ((ANNkd_leaf *)child[ANN_LO])->dump(out);

    if (!child[ANN_HI]->isLeaf)
        ((ANNkd_split *)child[ANN_HI])->dump(out);    // print high child
    else 
        ((ANNkd_leaf *)child[ANN_HI])->dump(out);
}
	...
	istream				&in)					// input stream for dump file
{
    ...
    ANNpoint the_bnd_box_lo;  // low bounding point
    ANNpoint the_bnd_box_hi;  // high bounding point
    ANNpoint the_pts;     // point storage
...
#define CHECK(x, y) if ((x) != (y)) \
	{ \
		cout << __FILE__ << ":" << __LINE__ << " Cannot read ann_1_1_char dump file format " << endl; \
		exit(1); \
	}
...
static ANNkd_ptr annReadDump(
	istream				&in,					// input stream
	ANNtreeType			tree_type,				// type of tree expected
	ANNpoint		&the_pts,				// new points (returned)
	ANNidxArray			&the_pidx,				// point indices (returned)
	int					&the_dim,				// dimension (returned)
	int					&the_n_pts,				// number of points (returned)
	int					&the_bkt_size,			// bucket size (returned)
...
#ifdef ANN_1_1_CHAR_DUMP
            // bulk load case
            CHECK(in.get(), ' '); // make sure there is a space
            annReadPtBatch(in, the_pts + (unsigned long long)idx*the_dim, 
                           the_dim);
#else
            for (j = 0; j < the_dim; j++) {
