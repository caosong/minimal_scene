...
#define PA(i,dim,d) (pa[(unsigned long long) pidx[(i)]*(dim) + (d)])

// accessing a single point
//#define PP(i)	(pa[pidx[(i)]])
#define PP(i,dim) (pa + (unsigned long long) pidx[(i)]*(dim))
...
double ann_1_1_char::annAspectRatio(
	int					dim,			// dimension
...
void ann_1_1_char::annEnclRect(
	ANNpoint pa,       // point array
	...
	ANNorthRect &bnds) // bounding cube (returned)
{
    for (int d = 0; d < dim; d++) {		// find smallest enclosing rectangle
        ANNcoord lo_bnd = PA(0,dim,d);		// lower bound on dimension d
        ANNcoord hi_bnd = PA(0,dim,d);		// upper bound on dimension d
        for (unsigned long long i = 0; i < (unsigned long long) n; i++) {
            if (PA(i,dim,d) < lo_bnd) lo_bnd = PA(i,dim,d);
            else if (PA(i,dim,d) > hi_bnd) hi_bnd = PA(i,dim,d);
        }
        bnds.lo[d] = lo_bnd;
        bnds.hi[d] = hi_bnd;
    }
...
void ann_1_1_char::annEnclCube(						// compute smallest enclosing cube
	ANNpoint		pa,				// point array
	ANNidxArray			pidx,			// point indices
	int					n,				// number of points
	int					dim,			// dimension
...
ANNdist ann_1_1_char::annBoxDistance(			// compute distance from point to box
	const ANNpoint		q,				// the point
	const ANNpoint		lo,				// low point of box
	const ANNpoint		hi,				// high point of box
	int					dim)			// dimension of space
{
    register ANNdist dist = 0;		// sum of squared distances
    register ANNdist t;
...
ANNdist ann_1_1_char::annSpread(				// compute point spread along dimension
	ANNpoint		pa,				// point array
	ANNidxArray			pidx,			// point indices
	int					n,				// number of points
	int					d)				// dimension to check
{
    ...
    ANNcoord min = PA(0,128,d);				// compute max and min coords
    ANNcoord max = PA(0,128,d);
    for (unsigned long long i = 1; i < (unsigned long long) n; i++) {
        ANNcoord c = PA(i,128,d);
        if (c < min) min = c;
        else if (c > max) max = c;
    }
    return ((ANNdist) max - (ANNdist) min);  // total spread is difference
}

void ann_1_1_char::annMinMax(					// compute min and max coordinates along dim
	ANNpoint		pa,				// point array
	ANNidxArray			pidx,			// point indices
	int					n,				// number of points
	int					d,				// dimension to check
...
{
    // KNS: note, hard-coding 128-dim features
    min = PA(0,128,d);						// compute max and min coords
    max = PA(0,128,d);
    for (unsigned long long i = 1; i < (unsigned long long) n; i++) {
        ANNcoord c = PA(i,128,d);
        if (c < min) min = c;
...
int ann_1_1_char::annMaxSpread(						// compute dimension of max spread
	ANNpoint		pa,				// point array
	ANNidxArray			pidx,			// point indices
	int					n,				// number of points
	int					dim)			// dimension of space
...
void ann_1_1_char::annMedianSplit(
	ANNpoint pa, // points to split
	ANNidxArray			pidx,			// point indices
	int					n,				// number of points
...
{
    // KNS: note, hard-coding 128-dim features
    long long l = 0;							// left end of current subarray
    long long r = n-1;						// right end of current subarray
    while (l < r) {
        register long long i = (r+l)/2;		// select middle as pivot
        register long long k;
        
        if (PA(i,128,d) > PA(r,128,d))			// make sure last > pivot
            PASWAP(i,r)
	
        PASWAP(l,i);					// move pivot to first position

        ANNcoord c = PA(l,128,d);			// pivot value
        i = l;
        k = r;
        for(;;) {  // pivot about c
            while (PA(++i,128,d) < c) ;
    ...
    if (n_lo > 0) {						// search for next smaller item
        ANNcoord c = PA(0,128,d);			// candidate for max
        int k = 0;						// candidate's index
        for (unsigned long long i = 1; i < (unsigned long long) n_lo; i++) {
            if (PA(i,128,d) > c) {
                c = PA(i,128,d);
                k = i;
            }
    ...
    cv = (PA(n_lo-1,128,d) + PA(n_lo,128,d))/2.0;
}
...
void ann_1_1_char::annPlaneSplit(  // split points by a plane
                                 ANNpoint pa,      // points to split
                                 ANNidxArray pidx, // point indices
                                 int n,            // number of points
                                 int d,            // dimension along which to split
                                 ...
                                 int &br2)         // second break (values == cv)
{
    ...
    for(;;) {							// partition pa[0..n-1] about cv
        while (l < n && PA(l,128,d) < cv) l++;
        while (r >= 0 && PA(r,128,d) >= cv) r--;
        if (l > r) break;
        PASWAP(l,r);
        l++; r--;
    }
...
void ann_1_1_char::annBoxSplit( // split points by a box
                               ANNpoint pa,  // points to split
                               ANNidxArray pidx,  // point indices
                               int n, // number of points
                               int dim,	// dimension of space
                               ...
                               int &n_in) // number of points inside (returned)
{
    ...
    for(;;) {							// partition pa[0..n-1] about box
        while (l < n && box.inside(dim, PP(l,dim))) l++;
        while (r >= 0 && !box.inside(dim, PP(r,dim))) r--;
        if (l > r) break;
        PASWAP(l,r);
        l++; r--;
    }
...
int ann_1_1_char::annSplitBalance(			// determine balance factor of a split
	ANNpoint		pa,				// points to split
	ANNidxArray			pidx,			// point indices
	int					n,				// number of points
	int					d,				// dimension along which to split
	ANNcoord			cv)				// cutting value
{
    // KNS: note, hard-coding 128-dim features
    int n_lo = 0;
    for(unsigned long long i = 0; i < (unsigned long long) n; i++) { // count number less than cv
        if (PA(i,128,d) < cv) n_lo++;
    }
    return n_lo - n/2;
}
...
void ann_1_1_char::annBox2Bnds(						// convert inner box to bounds
