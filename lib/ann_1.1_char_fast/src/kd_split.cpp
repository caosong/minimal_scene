...
void ann_1_1_char::kd_split(ANNpoint		pa,				// point array (permuted on return)
                               ...
                               int					&n_lo)			// num of points on low side (returned)
{
    ...
    ANNdist max_length = (ANNdist) bnds.hi[0] - (ANNdist) bnds.lo[0];
    for (d = 1; d < dim; d++) {			// find length of longest box side
        ANNdist length = (ANNdist) bnds.hi[d] - (ANNdist) bnds.lo[d];
        if (length > max_length) {
            max_length = length;
        }
    }
    ANNdist max_spread = -1;			// find long side with most spread
    for (d = 0; d < dim; d++) {
        // is it among longest?
    ...
    cut_val = ((ANNdist) bnds.lo[cut_dim] + (ANNdist) bnds.hi[cut_dim]) / 2;
...
void ann_1_1_char::sl_midpt_split(
                                  ANNpoint		pa,				// point array
                                  ANNidxArray			pidx,			// point indices (permuted on return)
                                  const ANNorthRect	&bnds,			// bounding rectangle for cell
                                  int					n,				// number of points
                                  ...
                                  int					&n_lo)			// num of points on low side (returned)
{
    ...
    ANNcoord ideal_cut_val = ((ANNdist)bnds.lo[cut_dim] + (ANNdist)bnds.hi[cut_dim])/2;
	...
	int					&n_lo)			// num of points on low side (returned)
{
    ...
    ANNdist max_spread = 0;			// find legal cut with max spread
    cut_dim = 0;
    for (d = 0; d < dim; d++) {
        ANNdist length = (ANNdist) bnds.hi[d] - (ANNdist) bnds.lo[d];
...
void ann_1_1_char::sl_fair_split(
	ANNpoint		pa,				// point array
	ANNidxArray			pidx,			// point indices (permuted on return)
	const ANNorthRect	&bnds,			// bounding rectangle for cell
	int					n,				// number of points
