...
ANNkd_leaf				*ann_1_1_char::KD_TRIVIAL = NULL;		// trivial leaf node
		...
		ostream &out)					// output stream
{
	...
	((ANNkd_split *) child[ANN_HI])->print(level+1, out);	// print high child
    } else {
	((ANNkd_leaf *) child[ANN_HI])->print(level+1, out);	// print high child
    }
	...
	((ANNkd_split *) child[ANN_LO])->print(level+1, out);	// print high child
    } else {
	((ANNkd_leaf *) child[ANN_LO])->print(level+1, out);	// print high child
    }
}
	...
	ANNorthRect			&bnd_box)				// bounding box
{
	...
	ch_stats.reset();							// reset
        ...
        if (!child[ANN_LO]->isLeaf) {
            ((ANNkd_split *) child[ANN_LO])->getStats(dim, ch_stats, bnd_box);
        } else {
            ((ANNkd_leaf *) child[ANN_LO])->getStats(dim, ch_stats, bnd_box);
        }
	...
	ch_stats.reset();							// reset
        ...
        if (!child[ANN_HI]->isLeaf)
            ((ANNkd_split *)child[ANN_HI])->getStats(dim, ch_stats, bnd_box);
        else
            ((ANNkd_leaf *)child[ANN_HI])->getStats(dim, ch_stats, bnd_box);
...
//----------------------------------------------------------------------
void ann_1_1_char::annClose()				// close use of ANN
{
	if (KD_TRIVIAL != NULL) {
		delete KD_TRIVIAL;
		KD_TRIVIAL = NULL;
...
ANNkd_ptr ann_1_1_char::rkd_tree(  // recursive construction of kd-tree
	ANNpoint    pa,   // point array
...
ANNkd_tree::ANNkd_tree( // construct from point array
	ANNpoint pa,    // point array (with at least n pts)
	int n,          // number of points
	int dd,         // dimension
	int bs,         // bucket size
