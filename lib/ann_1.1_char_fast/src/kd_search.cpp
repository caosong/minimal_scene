...
int ann_1_1_char::ANNkdDim;           // dimension of space
ANNpoint ann_1_1_char::ANNkdQ;        // query point
double ann_1_1_char::ANNkdMaxErr;     // max tolerable squared error
ANNpoint ann_1_1_char::ANNkdPts;      // the points
ANNmin_k *ann_1_1_char::ANNkdPointMK; // set of k closest points
...
void ANNkd_split::ann_search(ANNdist box_dist)
{
  ...
  ANNdist cut_diff = (ANNdist) ANNkdQ[cut_dim] - (ANNdist) cut_val;
  
  if (cut_diff < 0) { // left of cutting plane
      if (!child[ANN_LO]->isLeaf)
          ((ANNkd_split *)child[ANN_LO])->ann_search(box_dist);// visit closer child first
      else
          ((ANNkd_leaf *)child[ANN_LO])->ann_search(box_dist);

    ANNdist box_diff = (ANNdist) cd_bnds[ANN_LO] - (ANNdist) ANNkdQ[cut_dim];
    ...
    if (box_dist * ANNkdMaxErr < ANNkdPointMK->max_key()) {
        if (!child[ANN_HI]->isLeaf)
            ((ANNkd_split *)child[ANN_HI])->ann_search(box_dist);
        else
            ((ANNkd_leaf *)child[ANN_HI])->ann_search(box_dist);
    }
  } else { // right of cutting plane
      if (!child[ANN_HI]->isLeaf)
          ((ANNkd_split *)child[ANN_HI])->ann_search(box_dist); // visit closerchild first
      else
          ((ANNkd_leaf *)child[ANN_HI])->ann_search(box_dist);

    ANNdist box_diff = (ANNdist) ANNkdQ[cut_dim] - (ANNdist) cd_bnds[ANN_HI];
    if (box_diff < 0) // within bounds - ignore
      box_diff = 0;
    // distance to further box
    box_dist = (ANNdist) ANN_SUM(box_dist,
    ...
    if (box_dist * ANNkdMaxErr < ANNkdPointMK->max_key()) {
      if (!child[ANN_LO]->isLeaf)
          ((ANNkd_split *)child[ANN_LO])->ann_search(box_dist);
      else
          ((ANNkd_leaf *)child[ANN_LO])->ann_search(box_dist);
    }
  }
...
void ANNkd_leaf::ann_search(ANNdist box_dist)
{
  ...
  for (int i = 0; i < n_pts; i++) {  // check points in bucket
      pp = ANNkdPts + (unsigned long long) bkt[i] * ANNkdDim;  // first coord of next data point
