...
ANNpoint		ann_1_1_char::ANNkdFRQ;				// query point
...
namespace ann_1_1_char 
{
    ...
    ANNpoint ANNkdFRPts;				// the points
...
void ANNkd_split::ann_FR_search(ANNdist box_dist)
{
    ...
    ANNdist cut_diff = (ANNdist) ANNkdFRQ[cut_dim] - (ANNdist) cut_val;

    if (cut_diff < 0) {					// left of cutting plane
        if (!child[ANN_LO]->isLeaf)
            ((ANNkd_split *)child[ANN_LO])->ann_FR_search(box_dist);
        else
            ((ANNkd_leaf *)child[ANN_LO])->ann_FR_search(box_dist);

        ANNdist box_diff = (ANNdist) cd_bnds[ANN_LO] - (ANNdist) ANNkdFRQ[cut_dim];
        ...
        if (box_dist * ANNkdFRMaxErr <= ANNkdFRSqRad) {
            if (!child[ANN_HI]->isLeaf)
                ((ANNkd_split *)child[ANN_HI])->ann_FR_search(box_dist);
            else
                ((ANNkd_leaf *)child[ANN_HI])->ann_FR_search(box_dist);                
        }
        ...
        ANNdist box_diff = (ANNdist) ANNkdFRQ[cut_dim] - (ANNdist) cd_bnds[ANN_HI];
        if (box_diff < 0)				// within bounds - ignore
            box_diff = 0;
        // distance to further box
        box_dist = (ANNdist) ANN_SUM(box_dist,
...
void ANNkd_leaf::ann_FR_search(ANNdist box_dist)
{
    ...
    for (int i = 0; i < n_pts; i++) {	// check points in bucket
        ...
        pp = ANNkdFRPts + (unsigned long long) bkt[i] * ANNkdFRDim;		// first coord of next data point
